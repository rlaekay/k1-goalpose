...
> ⛔⛔ **2026-08-09 정정 (RETRACTIONS C37)**: 아래에서 발목 roll 의 **속도 초과**를
> 실기 리드로 취급한 부분은 **철회한다.** 같은 정책을 채점 물리만 바꿔 재면
> `over_share 0.5493 → 0.0000` 이다 — **`armature: 0` 인 채점 환경의 산물**이지
> 정책이 만든 것이 아니다. 실기 로그의 발목 roll **위치** 초과(±0.55 대 ±0.345)는
> **별개이고 여전히 유효**하다. 두 가지를 뭉치지 마라.

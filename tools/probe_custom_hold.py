...
from booster_robotics_sdk_python import (
    B1LocoClient, B1LowStateSubscriber, B1OdometerStateSubscriber, B1JointCnt,
...
class Probe:
    ...
    def legs(self):
        if self.low_state is None:
            return None
        return [self.low_state.motor_state_serial[i].q for i in range(11, 23)]
    ...
    def build_hold_cmd(self):
        ...
        motors = [MotorCmd() for _ in range(B1JointCnt)]
        self.low_cmd.motor_cmd = motors
        ...
        return [self.low_cmd.motor_cmd[i].q for i in range(B1JointCnt)]
...
def main():
    ...
    ap.add_argument("--hold", type=float, default=6.0,
                    help="seconds to stay in CUSTOM")
    ...
    try:
        ...
        if probe.low_state is None:
            print("!! no low_state -- robot stack is not running. aborting.")
            return 1

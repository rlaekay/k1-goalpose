...
안전
----
**이 프로세스는 읽기 전용이다.** `B1LowStateSubscriber` 하나만 만든다 -- publisher 도,
`ChangeMode` 도, `LowCmd` 도 없다. 로봇에 아무것도 명령하지 않으므로 Ctrl-C 로 언제든
끊어도 로봇 상태가 바뀌지 않는다.

⛔⛔ **그러나 "손으로 스톱까지 민다" 는 행위 자체가 안전하다는 뜻은 아니다.**
2026-08-09 에 이 측정을 한 직후 로봇이 힘이 풀리고 관절 보호(빨간불)에 들어갔다.
인과는 확정하지 못했지만 -- 유령 프로세스는 없었고 모션 스택은 떠 있었다 --
**관절을 펌웨어가 유효하다고 보는 범위(±0.345 rad) 밖으로 손으로 미는 것이
보호를 유발했을 가능성**을 배제할 수 없다. 실측 물리 스톱은 ±0.39~0.41 로
그 범위 밖이다.

그래서:
  * **한 번에 한 관절**, 천천히. 반동을 주지 마라.
  * 스톱에 닿으면 **버티지 말고 바로 놓아라.** 밀어붙인 채로 두지 마라.
  * 측정 뒤에는 **보호 상태인지 먼저 확인**하고 다음 단계로 가라.
  * 로봇이 보호에 들어가면 재부팅이 필요하다.

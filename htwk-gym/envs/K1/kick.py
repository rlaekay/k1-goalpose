"""K1 ball-kicking task (v5) -- port of envs/T1/kicking.py to K1.

Stand in front of a ball and kick it toward a world-frame target. Ball is a
second actor per env; obs adds the ball's XY position in the robot frame.

Differences from the T1 original (which stays frozen):
  - K1 constants: K1_locomotion.urdf, init z 0.58, base_height_target 0.52,
    terminate_height 0.35, PD gains Hip/Knee 100/2 (Ankle 50/1 unchanged).
  - Bug fixes: ball radius read from yaml (URDF sphere is 0.075, the T1 code
    hardcoded 0.05); kick target single-sourced from rewards.kick_target_pos_world
    (T1 hardcoded [5,0,0.05] in one reward, ignoring its yaml); ball spawn uses a
    proper local->world rotation of the offset (T1 rotated a [1,1,0] "forward"
    vector and multiplied elementwise -- balls spawned diagonally).
  - Debris removed: per-step prints, CSV logging, python-list ball-velocity
    stats, dead per-substep refresh_dof_force_tensor, dead cmd_resample timeout.
  - kick_success / fell logged via extras["rew_terms"] (summed per episode by
    the recorder -> clean tensorboard success/fall rates for free).

Trainer: existing train_v3.py (RunnerV3; no mirror maps here so the symmetry
loss auto-disables).
    python train_v3.py --task K1/Kick --headless True --sim_device cuda:0 --rl_device cuda:0
"""
...
class Kick(BaseTask):
    ...
    def _create_envs(self):
        ...
        # Ball asset (robot-agnostic; shared with the T1 task)
        ...
        # single source of truth for the ball's size (URDF sphere radius)
        self.ball_radius = ball_cfg["radius"]
        ...
        for i in range(self.num_envs):
            ...
            # robot first, ball second: actor indices robot=2i, ball=2i+1
    ...
    def _init_buffers(self):
        ...
        # two actors per env: [:, 0] robot, [:, 1] ball
        ...
        # curriculum stubs: unused by this task but read by RunnerV3's logging/save
        ...
        self.last_ball_lin_vel_world = torch.zeros_like(self.body_states[:, -1, 7:10])

    def _prepare_reward_function(self):
        """Prepares reward functions from all non-zero scales; ball_rolling_scale
        entries override a reward's scale while the ball is moving."""
    ...
    def _reset_dofs(self, env_ids):
        ...
        # 2 actors per env; even indices are the robots
    ...
    def _reset_root_states(self, env_ids):
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            ...
            actor_indices_to_update.shape[0],
        )

    def _reset_ball_at_robot_front(self, env_ids):
        """Place the ball at a randomized offset in the robot's local frame,
        properly rotated to world (the T1 original rotated a [1,1,0] vector and
        multiplied elementwise, spawning balls diagonally)."""
        if len(env_ids) == 0:
            return
        ...
        offset_local = torch.zeros(len(env_ids), 3, dtype=torch.float, device=self.device)
        offset_local[:, 0] = apply_randomization(offset_local[:, 0], self.cfg["randomization"].get("ball_init_pos_x"))
        offset_local[:, 1] = apply_randomization(offset_local[:, 1], self.cfg["randomization"].get("ball_init_pos_y"))
        offset_world = quat_rotate(robot_quat, offset_local)

        ball_target_xy = robot_pos[:, 0:2] + offset_world[:, 0:2]
        ball_target_z = self.terrain.terrain_heights(ball_target_xy) + self.ball_radius

        self.root_states[env_ids, 1, 0] = ball_target_xy[:, 0]
        self.root_states[env_ids, 1, 1] = ball_target_xy[:, 1]
        ...
        ball_actor_indices = (2 * env_ids + 1).to(dtype=torch.int32)
        self.gym.set_actor_root_state_tensor_indexed(
            self.sim,
            gymtorch.unwrap_tensor(self.root_states),
    ...
    def step(self, actions):
        ...
        self.time_since_ball_is_still_buf = torch.where(
            ball_is_active, torch.zeros_like(self.time_since_ball_is_still_buf), self.time_since_ball_is_still_buf + self.dt
        )
        self.time_since_ball_is_moving_buf = torch.where(
            ~ball_is_active, torch.zeros_like(self.time_since_ball_is_moving_buf), self.time_since_ball_is_moving_buf + self.dt
        )
        ...
        # ball-only resets (ball went stale but the robot is fine)
    ...
    def _check_termination(self):
        fell = self.root_states[:, 0, 7:13].square().sum(dim=-1) > self.cfg["rewards"]["terminate_vel"]
        fell |= self.base_pos[:, 2] - self.terrain.terrain_heights(self.base_pos) < self.cfg["rewards"]["terminate_height"]

        success = self.min_ball_vel_buf > np.ceil(self.cfg["rewards"]["min_ball_vel_s"] / self.dt)
        ...
        self.reset_buf |= self.time_since_ball_is_still_buf > self.cfg["rewards"].get("max_ball_still_time_s", 4.0)
        self.reset_buf |= self.time_since_ball_is_moving_buf > self.cfg["rewards"].get("max_ball_moving_time_s", 4.0)

        # per-episode success/fall rates on tensorboard for free: rew_terms are
        # summed over the episode by the recorder, and these fire on exactly one
        # step (the terminating one), so the episode sum is a clean 0/1.
        self.extras["rew_terms"]["kick_success"] = success.float()
        self.extras["rew_terms"]["fell"] = fell.float()

    def _compute_reward(self):
        ball_is_moving = torch.norm(self.ball_lin_vel, dim=-1) >= 0.1
    ...
    def _reward_tracking_lin_vel_x(self):
        # stand still while lining up: track zero velocity
    ...
    def _reward_ball_velocity_target_direction(self):
        """Ball velocity projected onto the ball->target direction, decayed the
        longer the ball has been moving (rewards the initial kick impulse)."""
        target_position = to_torch(self.cfg["rewards"]["kick_target_pos_world"], device=self.device).unsqueeze(0)
    ...
    def _reward_kicking_foot_approach_ball_stationary(self):
        """exp(-dist/sigma) of the closer foot to the ball (scale is swapped to 0
        via ball_rolling_scale once the ball moves)."""
        ...
        reward = torch.exp(-foot_ball_dist / proximity_sigma)

        max_reward = self.cfg["rewards"].get("max_approach_reward", 2.0)
        return torch.clamp(reward, min=0.0, max=max_reward)

    def _reward_body_alignment_for_kick(self):
        """Align the robot's forward vector with the direction to the kick target."""
        ...
        kick_target_pos_world = to_torch(self.cfg["rewards"]["kick_target_pos_world"], device=self.device).unsqueeze(0)
    ...
    def _reward_body_angle(self):
        base_pitch, base_roll, _ = get_euler_xyz(self.base_quat)
        pitch_normalized = (base_pitch + torch.pi) % (2 * torch.pi) - torch.pi
        roll_normalized = (base_roll + torch.pi) % (2 * torch.pi) - torch.pi
        penalty = torch.square(torch.abs(pitch_normalized)) + torch.square(torch.abs(roll_normalized))
        return 1.0 / (0.1 + penalty**2) - 1.0

    def _reward_ball_acceleration(self):
        """Forward ball acceleration minus sideways component: rewards a clean
        kick impulse in the target direction."""
        ...
        return torch.tanh(torch.clamp(ball_effective_acceleration, min=0.0) / acceleration_scale) * max_acceleration_reward

    def _reward_waiting(self):
        """Grows quadratically with time; negative scale punishes dawdling
        (swapped to 0 via ball_rolling_scale once the ball moves)."""

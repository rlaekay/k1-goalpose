...
class GoalPose(BaseTask):
    ...
    def _reward_feet_cross(self):
        ...
        ⛔ **이 결함이 관측된 arm 차이를 만들었다고 주장하지 마라**(분석 세션 지적,
        2026-08-09). `N3_pathcross`(-20)가 `N1_path` 보다 좁게 지령하는 것은 사실이지만
        (명령 p1 +0.049 대 +0.087, 5 eval 시드 전부), **그 원인이 이 항이라는 근거는 없다**:
          * 작동점 s ~ 0.18 에서 벌칙도 기울기도 **정확히 0** 이다. 압력은 꼬리에만 있다.
          * 실측 기준 꼬리 체류가 **위 0.086 % / 아래 0.000 %**(`N1`) 로 거의 0 이다
            ⇒ 이 항은 **거의 항상 정확히 0** 이라 그만한 변화를 만들 기울기가 없다.
          * arm 당 **학습 실행이 1개**라 학습 시드 산포가 측정된 적이 없다.
        ⇒ *"두 정책이 다르다"* 는 확정이고 *"이 레버가 그렇게 만들었다"* 는 **미확정**이다.
        고치는 근거는 결과가 아니라 **식 자체**다 -- 실기 p1 -16.35 cm 가 무벌칙 구간이다.

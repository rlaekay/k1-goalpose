...
def main():
    ...
    if not allrows:
        ...
        return 0
    print("=" * 78)
    sym = [r[0] for r in allrows]          # 대칭 가정 = (b)에 가장 유리
    use = float(np.median([s[3] for s in sym]))
    print("판정 — 대칭 감속 가정(구간 길이 한계 쪽에 가장 유리한 가정) 기준")
    print("  활용률 median %.0f%%" % (100 * use))
    if use >= 0.85:
        print("  => 관측이 이론상한에 붙어 있다. 0.40 s는 구간 길이의 한계다.")
        print("     정책은 이미 낼 수 있는 만큼 낸다 -> R4는 sprint 카테고리로 충분하다.")
    elif use <= 0.6:
        print("  => 거리 여유가 있는데 안 쓴다. 정책/보상 쪽 한계다.")
        print("     -> R4는 과제 분포만으로 부족하고 보상을 함께 봐야 한다.")
    else:
        print("  => 중간이다. 거리도 정책도 단독 원인이 아니다.")
        print("     -> sprint를 넣되 순항이 실제로 늘어나는지 같은 지표로 재확인할 것.")
    print("  주의: 가속도는 능력이 아니라 행동의 측정이다. 지금 과제는 최대 요구속도가")
    print("        0.62 m/s라 빨리 가속할 이유가 없다. sprint 아래에서는 가속도 자체가")
    print("        달라질 수 있고, 그러면 이 상한도 올라간다.")
    return 0

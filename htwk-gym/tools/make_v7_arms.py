...
# ---- NM: 도착은 하는데 **안 선다** (2026-08-08, §8-56) ------------------------
#
# `NH_zeroclock` 의 정확도 리포트를 읽고 나서야 병목이 바뀐 것을 알았다. NZ 때와
# 완전히 다른 그림이다:
#
#                        NZ_zeroiid      NH_zeroclock
#   최근접 거리 median      20.3 cm        **3.9 cm**   <- 이미 게이트 안에 들어간다
#   횡방향 |오차| median     0.17 m         0.04 m      <- 보행시계 수정이 횡방향을 지웠다
#   최종 위치오차 median     25.6 cm        9.33 cm
#   성공률(완화 10cm)        10.2 %         52.8 %
#   성공률(엄격 5cm+정지)     8.6 %         18.9 %
#
# ⭐ **로봇은 3.9 cm 까지 들어갔다가 9.3 cm 로 다시 나온다.** 그리고 리포트가 그 이유를
# 직접 찍는다: *"도착 후 안 멈춤 -- 20 % 가 위치·방향은 맞았는데 속도 > 0.10 m/s"*.
# 완화(52.8 %)와 엄격(18.9 %)의 34 포인트 차이가 대부분 **정지 조건**이다.
#
# 즉 남은 문제는 "가까이 가기"가 아니라 **"가서 서기"** 다. 그런데 그것을 보상하는
# 항의 스케일이 **0** 이다:
#   `goal_stop: 0.0`  (`_reward_goal_stop` = 목표 반경 안에서의 잔여 속도 벌칙)
#   `goal_position: 0.0` · `goal_heading: 0.0` -- 목표 보상은 constellation + goal_reached 뿐
# `goal_reached`(1.0)는 "반경 0.1 m 안 **이고** 정지" 일 때만 켜지는 계단이라,
# 그 문턱에 못 미치는 구간에는 **속도를 줄일 이유가 전혀 없다.**
#
# 그래서 NM = NH + `rewards.scales.goal_stop` 0.0 -> -3.0 (레버 하나).
# 크기 근거: 벌칙 = 3.0*(|v_xy|^2 + w_z^2). 반경 안에서 0.3 m/s 면 0.27/step 로
# 도착 보너스(1.0/step)의 27 %, 0.5 m/s 면 75 % 다. 즉 "천천히 지나가기"가 "서기"보다
# 손해가 되는 지점이 0.3~0.5 m/s 사이에 온다. 리포트의 자동 제안(-3)과도 같다.
#
# 판정(사전 등록). 대조군은 `NH_zeroclock`(9.33 cm / strict 18.9 % / 낙상간격 27,300 s):
#   * strict > 18.9 % 이고 위치오차 < 9.33 cm, 낙상간격이 27,300 s 의 절반 이상
#       -> 병목이 정지 조건이 맞다. **5 cm 게이트 통과 후보 1순위.**
#   * strict 만 오르고 위치오차가 그대로면 -> 정지는 고쳤지만 마지막 몇 cm 는 다른 문제다.
#     그때 `goal_position_sigma`(현재 1.0, 근거리에서 평평)가 다음 레버다.
#   * 낙상간격이 크게 떨어지면 -> 목표 근처에서 급정거를 배운 것이다. 벌칙을 -1 로 낮춰
#     다시 잰다.
NM_ARMS = {
    "NM_goalstop": merge(_N_BASE, _PATH_ON, _ZERO_ON,
                         {"commands.path.pause_gait_during_dwell": True,
                          "rewards.scales.goal_stop": -3.0}),
}
N_ARMS.update(NM_ARMS)


# ---- NP: 정확도 챔피언 + 안정성 레버 (2026-08-08, §8-56) ---------------------
#
# 두 갈래가 각각 자기 축에서 최고를 냈다. 아직 합친 적이 없다.
#
#   `NH_zeroclock` (계보 A) -- **정확도** 9.33 cm / strict 18.9 % / 낙상간격 27,300 s
#   `NJ_armasset`  (계보 B) -- **안정성** 자기 물리에서 **낙상 0**, 순항체류 49.0 %,
#                              속도 1.40 m/s, 그리고 **발이 한 번도 교차하지 않는다**
#                              (접촉체류 0.00 %, 최소간격 +5.21 cm)
#
# NP = NH + `asset.armature_by_joint`(자산 분포) 레버 **하나**. 계보는 A 로 둔다
# (warm start = I3b_stance10/model_200, NH 와 같은 지점). 대조군은 `NH_zeroclock`.
#
# ⚠️ 채점을 **두 물리에서** 해야 한다(§8-52·§8-56 에서 두 번 확인한 함정):
#   (1) 공통 프로토콜(armature 없음) -- 다른 모든 arm 과 비교 가능한 값
#   (2) 자기 분포(발목 0.05)         -- 레버를 켠 값
# 한쪽만 재면 §8-52 의 NG@0.0(낙상간격 1.5 s)·§8-56 의 NJ@0.0(1 s)과 같은 함정이다.
# 이식은 `tools/eval_cfg_override.py` 가 한 키만 옮기고 나머지가 바뀌면 죽는다.
#
# 판정(사전 등록):
#   * 자기 물리에서 정확도 <= 9.33 cm 이고 순항체류 >= 49.0 % 이고 낙상 0 근처
#       -> 두 축이 합산된다. **배포 후보 1순위이고 격차가 크다.**
#   * 정확도가 크게 나빠지면 -> armature 와 정확도 레버(영점·보행시계)가 경쟁한다.
#     그때는 armature 를 커리큘럼(0 -> 자산 분포)으로 넣는 쪽을 시도한다.
#   * 안정성이 NJ 만 못하면 -> 이득이 계보 B(NE 기반)에 특이했던 것이다.
NP_ARMS = {
    "NP_clockarm": merge(_N_BASE, _PATH_ON, _ZERO_ON,
                         {"commands.path.pause_gait_during_dwell": True},
                         _ARM_ASSET),
}
...
#
# ⛔⛔ **"벤더로 올리면 해결" 이 아니다 — "벤더가 도달 가능한 최선" 이다**
#    (2026-08-10, MJC 직진 재측정 · 발목 p99 를 5셀 전부 15.0 으로 고정한 한 요인 설계):
#
#        URDF 30·20·20·40  ->  낙상 **66.0**/120 s
#        MJCF 45           ->  22.8
#        **벤더 68·76·38·112 -> 4.2**
#        벤더 x1.5         ->  **0.0**
#        벤더 x2.0         ->  **0.0**
#
#    ⇒ effort 축은 살아남았고 크기가 60배 -> **16배** 로 줄었다(원 그리는 판 -> 직진 판).
#    ⛔ 그런데 **벤더에서도 4.2 가 남고 x1.5 에서야 0 이 된다** = **정책이 실제 모터가
#      낼 수 있는 것보다 더 원한다.** 이건 **하드웨어 한계 서술**이라 학습으로 못 넘는다.
#    ⇒ **`NS` 의 목표는 "제약을 없애는 것" 이 아니라 "정책이 그 한계 안에서 도는 보행을
#      배우게 하는 것" 이다.** effort 를 올려 주는 것만으로 4.2 -> 0 이 되지 않는다.
#
# ---- 발목을 20 으로 두는 것은 **의도적 선택**이다 (분석 세션 질문에 대한 답) ------
#
# 로봇의 발목 박스는 **17** 이다(`Goal_Pose_E0.yaml:244`). 학습은 URDF **20** 이라
# 3 N·m 어긋나고 그것이 창 폭(5)의 60 % 다. 그래도 20 을 쓴다. 이유 셋:
#
#   1. **레버를 하나로 지킨다.** 발목을 20->17 로 같이 바꾸면 `NS` 는 두 레버가 되고
#      결과 귀속이 다시 불가능해진다 -- C55 가 정확히 그 실패의 기록이다.
#   2. **어긋남의 방향이 안전한 쪽이다.** 배포(17)가 학습(20)보다 **더 조이면**
#      정책은 가끔 17~20 을 요구하고 17 을 받는다 = 포화하는 결핍이다. 반대
#      (배포가 더 느슨)면 정책이 **학습한 적 없는 권한**을 받아 불안정해진다.
#   3. **MJC 창은 `I3b` 위에서 잰 값이다.** *"그 값으로 학습하면 최적"* 이 아니라
#      *"그 정책에 그 상한을 걸면 최적"* 이다. 학습값으로서 17 이 맞는지는 **미측정**이고,
#      `NS` 가 성공하면 그때 `20 -> 17` 이 **깨끗한 후속 한 레버**가 된다.
#
# ⇒ 배포 쪽에는 대안을 알렸다: **박스를 20 으로 올리면** 학습과 정확히 맞고 창 안에도
#   있다(여유가 5 -> 2 로 줄 뿐). 마진과 일치 중 무엇을 살지는 배포 판단이다.
# ⛔⛔ **2026-08-09 축소 — 구속되는 관절만 올린다.** `NQ@vendor` 5시드 실측 `sat_share`:
#
#     Left/Right_HipP   30 (벤더 68)    **7.42 / 5.16 %**   <- 물린다
#     Left/Right_HipY   20 (벤더 38.3)  **5.64 / 4.50 %**   <- 물린다
#     Left/Right_KneeP  40 (벤더 112)    0.059 / 0.020 %    <- **안 물린다**(요구 8.2 N·m)
#     Left/Right_HipR   35 (벤더 76)     0.002 / 0.001 %    <- **안 물린다**(요구 5.1 N·m)
#
# **구속되지 않는 제약을 풀면 최적해가 안 바뀐다.** 무릎·힙롤을 같이 올리면 GPU 만
# 태우고 레버 수만 늘린다. ⇒ **힙 pitch·yaw 만** 올린다.
# (분석 세션이 MJC 의 `NQ` 스윕을 근거로 축 전체를 철회했는데, 그 스윕은 **낙상** 축이고
#  **이미 학습된 정책**에 상한을 바꿔 낀 것이다. 학습 중 구속은 별개이고 -- 위 표가
#  그 구속을 직접 잰 값이다 -- 힙 pitch·yaw 는 4.5~7.4 % 로 실재한다.)
_EFFORT_VENDOR = {
    "asset.torque_limits_by_joint": {
        "Hip_Pitch": 68.0,
        "Hip_Yaw": 38.3,
        # ⛔ Knee_Pitch / Hip_Roll 은 **일부러 뺐다** -- 구속되지 않아 no-op 이다.
        # ⛔ Ankle_* 도 비워 둔다 = URDF 20 유지 = MJC 안전창 [15,20] 안.
    },
}
...
# ⛔⛔ **새 arm 은 반드시 이 줄 위에 정의해라.** `ALL_ARMS` 는 여기서 각 dict 를
# **값으로 펼쳐** 만들어지므로, 아래에서 `N_ARMS.update(...)` 를 해도 **전파되지 않는다.**
# 2026-08-09 에 `NS_effort` 가 정확히 그렇게 누락됐고(`--only NS_effort` 가 KeyError),
# 학습 잡의 가드가 잡아서 `rc1` 로 죽었다. 가드가 없었으면 조용히 다른 arm 을 돌렸다.
